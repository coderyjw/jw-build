  ...
  startServer = async () => {
    ...
    try {
      ...
      compiler = webpack(webpackConfig, (err, stats) => {
        if (err) {
          ...
          if (res.errors && res.errors.length > 0) {
            ...
            log.info(
              "COMIPLE SUCCESSFUL!",
              "Comile finish in " + res.time / 1000 + "s"
            );
          }
        }
      });
      ...
      if (devServer.startCallback) {
        ...
        devServer.listen(serverConfig.port, serverConfig.host, startCallback);
      }
      function startCallback(err) {
        if (err) {
          log.error("WEBPACK_DEV_SERVER ERROR!");
          log.error("ERROR MESSAGE", err.toString());
        } else {
          log.info("WEBPACK_DEV_SERVER LAUNCH SUCCESSFUL");
        }
      }
    } catch (err) {
